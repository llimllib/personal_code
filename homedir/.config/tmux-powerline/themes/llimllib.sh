...
# color documentation: https://www.ditig.com/256-colors-cheat-sheet
BG=235
FG=255
BLUE=39
BLACK=0
...
#
# some commands from the default that I'm not using:
# "ifstat 30 255" \
# "ifstat_sys 30 255" \
...
if [ -z "$TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS" ]; then
	TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS=(
		"pwd $BLUE $BLACK" \
		"vcs_branch $YELLOW $BLACK" \
		"macos_notification_count 29 255" \
		"battery $MAGENTA $BLACK" \
		"date_day $RED $BLACK" \
		"date $RED $BLACK ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}" \
		"time $RED $BLACK ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}" \
	)
fi
