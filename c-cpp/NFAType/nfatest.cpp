...
int main(int argc, char *argv[])
{
  ...
  /***** test *******/
  stateSet = new SetType("tuvwx");
  alphaSet = new SetType("01");
  acceptSet = new SetType("uw");
  const char* trans = "t-e,u,e,w;u-1,v;v-1,u;w-0,x;x-0,w;";
  
  f = new NFAType(stateSet, alphaSet, trans, 't', acceptSet);
  
  bool test = f->is_accepted("00");
  
  if(test) cout << "accepted\n";
  else     cout << "not accepted\n";
  /***** /test *******/
  
  fin.open("nfadata.dat");
  ...
  while(fin) {
    ...
    f = new NFAType(stateSet, alphaSet, transition.c_str(), 
                    start[0], acceptSet);
