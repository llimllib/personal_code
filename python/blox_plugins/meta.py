...
#       PRO: works with less maintenance
#       CON: significantly slower if there's a whole lot of files
#       Q: make it a config option? That makes this more complicated.
...
#tools.make_logger('/home/sites/llimllib/www/public_html/blog/log/out.log')
#log = tools.log
...
def metasearch(keyword, fname=None):
    ...
    if type(fname) == type(''):
        ...
        up = cPickle.Unpickler(file(pickle, 'rb'))
        return up.load().get(keyword, '')
...
def walkgetmeta(data, dirname, fnames):
    ...
    for f in fnames:
        if f.endswith('.meta'):
            up = cPickle.Unpickler(file(pathjoin(dirname, f), 'rb'))
            md = up.load()
    ...
    for f in fnames:
        if f.endswith('.txt'):
            ...
            if not os.path.isfile(pickle):
                ...
                pickle = file(pickle, 'wb')
                #TODO: use protocol 1 or 2 to speed it up?
                cp = cPickle.Pickler(pickle, -1)
                cp.dump(md)
                pickle.close()
