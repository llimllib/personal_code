...
class _PathDrawer:
    ...
    def drawPaths(self, canvas, linewidth, borderwidth, width, height, paths):
        ctx = canvas.ctx()
        ...
        for n, path in enumerate(paths):
            for i in self.lineCoords(path, len(paths)):
                ctx.line_to(width * i[0], linewidth + height * i[1])
            ctx.set_source_rgb(*self.getColor(n, len(paths)))
...
class Weave(_PathDrawer):
    ...
    def draw(self, paths, title, fname, linewidth, borderwidth, rotate=False):
        c = Canvas(self.width, self.height)
        self.drawPaths(c, linewidth, borderwidth, self.width, self.height, paths)
        c.save(fname, rotate)
